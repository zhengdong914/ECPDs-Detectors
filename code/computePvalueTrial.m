...
function z_scores= computePvalueTrial(onlineDecodeResult,seq,options,onlineDecodeResult1)

baselineRange = [(options.baseline(1)/options.binsize): (options.baseline(2)/options.binsize)];

for j=1%:length(seq.stimulus)
    if options.preTrial || options.currentTrial || strcmp(seq.stimulus{j},'SIM')
       trials = eval(['seq.ntrial' seq.stimulus{j}]);
    else
       trials = eval(['seq.ntrial' seq.teststimulus{j}]);
%        trials = eval(['length(onlineDecodeResult.' options.decoders{1} '.' seq.region{1} seq.stimulus{j} ')-1']);
    end
    st = 1;
    et = trials;
    ...
    for i=1:length(seq.region)
        if(i==2)
           options.confidenceInterval=3;
        else
           options.confidenceInterval=2;    % for ACC could be 2
        end
        for d=1:length(options.decoders)
            for k=st:et
                x = eval(['onlineDecodeResult.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '('  num2str(k) ').x']);
                V = eval(['onlineDecodeResult.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '('  num2str(k) ').V']);
                x_error=zeros(size(x));
                if ~isempty(V) && length(V)>1
                    if nargin==3
                        mean_base = mean(x(:,baselineRange),2);
                    else
                        tmp = eval(['onlineDecodeResult1.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '('  num2str(k) ').x']);
                    ...
                    if(size(x,1)==2)
                       ...
                       eval(['z_scores.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '(' num2str(k) ').zscore = z_score']);
                       eval(['z_scores.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '(' num2str(k) ').upperBound = upperBound']);
                       eval(['z_scores.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '(' num2str(k) ').lowerBound = lowerBound']);
                    end
                    eval(['z_scores.' options.decoders{d} '.' seq.region{i} seq.stimulus{j} '(' num2str(k) ').pValue = pValue']);
                else
    ...
    if (size(seq.region,2)==2)&&(j==1)
          for d=1:length(options.decoders)
            if ~(strcmp(options.decoders{d},'GT'))
                baseline = [options.baseline(1)/options.binsize:options.baseline(2)/options.binsize];
                for k=st:et
                    xACC_z=eval(['z_scores.' options.decoders{d} '.' seq.region{1} seq.stimulus{j} '(' num2str(k) ').zscore;']);
                    xACC_lowb=eval(['z_scores.' options.decoders{d} '.' seq.region{1} seq.stimulus{j} '(' num2str(k) ').lowerBound;']);
                    xACC_upb=eval(['z_scores.' options.decoders{d} '.' seq.region{1} seq.stimulus{j} '(' num2str(k) ').upperBound;']);
                    xS1_z=eval(['z_scores.' options.decoders{d} '.' seq.region{2} seq.stimulus{j} '(' num2str(k) ').zscore;']);
                    xS1_lowb=eval(['z_scores.' options.decoders{d} '.' seq.region{2} seq.stimulus{j} '(' num2str(k) ').lowerBound;']);
                    xS1_upb=eval(['z_scores.' options.decoders{d} '.' seq.region{2} seq.stimulus{j} '(' num2str(k) ').upperBound;']);
                    xACC=xACC_z;
                    xS1=xS1_z;
...
%                           xACC(i)=xACC(i)/(xACC_upb(i)-xACC_lowb(i));
                    ...
                    ccthresh = 0;%options.threshold-1;
                    for m=1:size(xS1,2)
...
%                             xACC(m)=(ccthresh+(abs(xACC(m)-ccthresh)).^.3)*xACC(m)/abs(xACC(m));
                    ...
                    if ~isempty(xx)
                        ...
                        options.C_thresh=[mean_base-options.ccconfidenceInterval*std_base mean_base+options.ccconfidenceInterval*std_base];
                    else
                        mean_base=[];
                        std_base=[];
                        options.C_thresh=[];
...
%                         options.crossvalue=[];
%                     end
                    eval(['z_scores.' options.decoders{d} '.' seq.region{1} seq.stimulus{j} '(' num2str(k) ').ccvalue = cct']);
                    eval(['z_scores.' options.decoders{d} '.' seq.region{1} seq.stimulus{j} '(' num2str(k) ').ccthresh = options.C_thresh']);
                    eval(['z_scores.' options.decoders{d} '.' seq.region{2} seq.stimulus{j} '(' num2str(k) ').ccvalue = cct']);
                    eval(['z_scores.' options.decoders{d} '.' seq.region{2} seq.stimulus{j} '(' num2str(k) ').ccthresh = options.C_thresh']);
                end
            end
         end
    end
