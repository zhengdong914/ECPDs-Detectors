...
% author: Sile Hu
% date: 2017-3-13
function drawZscorePlots_SIM_subset(z_scores,plotseq,options)
...
for s=1:length(stimulus)
    ...
    for t=st:et
        ...
        subplotIdx = '311';
        seqall = eval(['plotseq(1).' seq.region{1} stimulus{s} '(trials(t))']);
        if options.currentTrial || strcmp(seq.stimulus{s},'SIM')
...
%                 seqall.y = [seqall.y;eval(['seq.' seq.region{2} options.teststimulus{s} '(trials(t)).y'])];
        ...
        subplotIdx = '312';
        seqall = eval(['plotseq(2).' seq.region{1} stimulus{s} '(trials(t))']);
        drawRasterTrialNew(seqall,titleStr,edges,subplotIdx); 
%         subplotIdx = '312';         
% 
...
%         end
        ...
        for r=1:length(z_scores)
            if (r~=1)
                for d=1:length(decoders)
                    if ~(strcmp(decoders{d},'GT'))
                        if (length(seq.region)==1)
                            z_score = eval(['z_scores(r).' decoders{d} '.' seq.region{1} stimulus{s} '(trials(t));']);
                        else
                            z_score = eval(['z_scores(r).' decoders{d} '.' seq.region{2} stimulus{s} '(trials(t));']);
                        end
